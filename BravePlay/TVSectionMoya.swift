...
public enum  TVSection {
    //视频
    case TVVideos(page: Int)
}

extension TVSection :TargetType {
    ...
    public var parameters: [String: AnyObject]? {
        switch self {
        case .TVVideos(let page):
            return ["expand" : ["topic","channel"],"subVersion":"0.2","page":"\(page)"]
        }
    }
...
let tvEndpointClosure = { (target: TVSection) -> Endpoint<TVSection> in
    let endpoint: Endpoint<TVSection> = Endpoint<TVSection>(URL: url(target),
...
let TVSectionProvider = MoyaProvider<TVSection>(endpointClosure:tvEndpointClosure,
